...
const youSheBiaoTiHei = localFont({
  src: '../app/fonts/YouSheBiaoTiHei-2.ttf',
  display: 'swap'
})

const pingFangSC = localFont({
  src: '../app/fonts/PingFang SC Regular.ttf',
  display: 'swap'
})

const alibabaPuHuiTiM = localFont({
  src: '../app/fonts/Alibaba PuHuiTi M.ttf',
  display: 'swap'
})

interface RealTimeCheckModalProps {
  inspectionId: string | number
  onClose: () => void
  isOpen: boolean
}

export function RealTimeCheckModal({ inspectionId, onClose, isOpen }: RealTimeCheckModalProps) {
  // 状态管理 - 存储所有字段的值
  ...
  // 根据 inspectionId 获取数据
  useEffect(() => {
    if (inspectionId && isOpen) {
      fetchInspectionData()
    }
  }, [inspectionId, isOpen])
  ...
  const handleSave = async () => {
    ...
    // 构建请求数据
    const requestData = {
      inspectionId: parseInt(inspectionId.toString()),
      changeReason: "数据修正",
      modifyFields
    }

    try {
      const response = await apiFetch('/api/api/inspection/modify', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    >
      <div 
        className="relative max-h-[90vh] overflow-auto bg-slate-950 rounded-lg border border-cyan-500/40 shadow-2xl"
      ...
      >
        ...
        <button
          onClick={onClose}
          className="sticky top-2 right-2 float-right z-50 w-10 h-10 flex items-center justify-center rounded-full bg-slate-800/80 hover:bg-slate-700/80 border border-cyan-500/40 transition-all text-cyan-400 hover:text-cyan-300 text-2xl font-bold mr-2"
        >
          ×
        </button>
        ...
        <div className="px-4 pt-2 pb-4">
          ...
          {/* 左侧主要内容 */}
          <div className="flex-1 space-y-3" style={{ maxWidth: '1240px' }}>
            {/* 详情照片区域 */}
            <div className="space-y-1.5">
              <h2 className={`${youSheBiaoTiHei.className} font-medium text-white px-2`} style={{ fontSize: '14px' }}>
                详情照片
              </h2>
            ...
            <div className="flex gap-1.5 items-start">
              ...
              >
                <h3 className={`${youSheBiaoTiHei.className} font-medium text-white mb-1.5`} style={{ fontSize: '14px' }}>
                  车身侧面图像
                </h3>
          ...
          {/* 右侧图像区域 - 两列六行 */}
          <div className="flex flex-col gap-2" style={{ marginTop: '34px', flexShrink: 0 }}>
